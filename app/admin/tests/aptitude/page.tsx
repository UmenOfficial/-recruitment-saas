...
import { fetchAptitudeTestsAction, createAptitudeTestAction, updateAptitudeTestAction, deleteAptitudeTestAction } from './actions';
...
export default function AptitudeTestManagement() {
    ...
    const fetchTests = async () => {
        setLoading(true);
        try {
            const res = await fetchAptitudeTestsAction();
            if (res.success) {
                setTests(res.data || []);
            } else {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            const res = await createAptitudeTestAction(newTest.title, newTest.description, newTest.time_limit);
            if (res.success) {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await deleteAptitudeTestAction(id);
            if (res.success) {
    ...
    // ... existing handleCreate (removed duplicate in replacement) ...
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            const res = await updateAptitudeTestAction(
                editingTest.id,
                editingTest.title,
