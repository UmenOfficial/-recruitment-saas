...
async function forceRecalcFixed() {
    console.log('--- Force Recalculating Scores (FIXED LOGIC) ---\n');
    ...
    for (const test of targetTests) {
        ...
        for (const res of results) {
            ...
            // 2. COMPETENCIES
            let totalRaw = 0; // Accumulate Competency T-scores

            // Filter out TOTAL from comp loop first
            ...
            compsToCalc.forEach(norm => {
                ...
                let compRaw = 0; // Sum of Scale T-scores
            ...
            if (totalNorm) {
                ...
                detailedScores['total'] = {
                    raw_score: parseFloat(totalRaw.toFixed(2)),
                    t_score: parseFloat(finalT.toFixed(2))
                };
                // Add explicit norm key too
                detailedScores[totalNorm.category_name] = detailedScores['total'];
            }
